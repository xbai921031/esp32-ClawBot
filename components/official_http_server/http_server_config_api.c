...
static esp_err_t config_post_handler(httpd_req_t *req)
{
    ...
    /* Log which config group was saved (BEFORE free) */
    bool saved_wifi = false, saved_llm = false, saved_im = false;
    for (size_t i = 0; i < CONFIG_FIELD_COUNT; i++) {
        const config_field_def_t *field = &CONFIG_FIELDS[i];
        cJSON *item = cJSON_GetObjectItemCaseSensitive(root, field->name);
        if (cJSON_IsString(item)) {
            if (strcmp(field->group, "wifi") == 0) saved_wifi = true;
            if (strcmp(field->group, "llm")  == 0) saved_llm  = true;
            if (strcmp(field->group, "im")   == 0) saved_im   = true;
        }
    }
    if (saved_wifi) ESP_LOGI(TAG, "[CONFIG] WiFi saved (SSID: %s)", config->wifi_ssid);
    if (saved_llm)  ESP_LOGI(TAG, "[CONFIG] LLM  saved (backend: %s model: %s)",
                              config->llm_backend_type, config->llm_model);
    if (saved_im)   ESP_LOGI(TAG, "[CONFIG] IM   saved (wechat_account: %s)",
                              config->wechat_account_id);
    ...
    if (!resp) {
        ...
        http_server_json_add_string(resp, "message",
                                    "Saved. Device will restart automatically.");
        http_server_send_json_response(req, resp);
    }

    if (ctx->services.restart_device) {
