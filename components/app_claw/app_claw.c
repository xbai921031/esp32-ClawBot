                                      ...
                                      claw_core_config_t *core_config)
{
    ...
    core_config->task_stack_size = 10 * 1024;
...
esp_err_t app_claw_start(const app_claw_config_t *config)
{
    ...
    const uint32_t max_tool_iterations = 10;
#endif
#if CONFIG_APP_CLAW_CAP_EVENT_ROUTER
...
#endif
#if CONFIG_APP_CLAW_CAP_MEMORY
    esp_err_t mem_err = init_memory(config, &paths, max_tool_iterations);
    if (mem_err != ESP_OK) {
        ESP_LOGW(TAG, "Memory init non-fatal (%s), continuing without memory", esp_err_to_name(mem_err));
    }
#endif
#if CONFIG_APP_CLAW_CAP_SESSION_MGR && (CONFIG_APP_CLAW_CAP_MEMORY || CONFIG_APP_CLAW_CAP_SKILL_MGR)
